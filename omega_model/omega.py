    ...
    if all(s in omega_globals.options.MarketClass.responsive_market_categories for s in children):
        ...
        if all(mc in market_class_data[compliance_id] for mc in cross_subsidy_pair):
            ...
            cross_subsidy_options_and_response, iteration_log = \
                search_cross_subsidies(calendar_year, compliance_id, node_name, cross_subsidy_pair,
                                       producer_decision,
                                       cross_subsidy_options_and_response, producer_consumer_iteration_num,
                                       iteration_log)
...
def update_cross_subsidy_pair_console_log(cross_subsidy_pair, share_convergence_error, cross_subsidy_pricing_error,
                                          mcat_converged):
    """
...
def run_omega(session_runtime_options, standalone_run=False):
    ...
    try:
        ...
        for omega_globals.pass_num in range(len(consolidate)):
            ...
            # Initilize automaker_cv_cost_curve dataframe (see full_industry_vehicles.py)
            if session_runtime_options.consolidate_manufacturers == False:
                omega_globals.industry_v_df = pd.DataFrame()
                
            omega_globals.automaker_v_cost_curve = pd.DataFrame()
            # else:
            # omega_globals.consolidate_v_cost_curve = pd.DataFrame()
            ...
            if not init_fail:
                ...
                if session_runtime_options.consolidate_manufacturers == False:
                    full_industry_vehicles.save_automaker_cv_cost_curve()
                    full_industry_vehicles.save_automaker_v_cost_curve(False)
                else:
                    full_industry_vehicles.save_automaker_v_cost_curve(True)
